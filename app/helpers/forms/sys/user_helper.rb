...
module Forms::Sys::UserHelper
  ...
  def sys_user_view(user,opts={})
    ...
    view_for user, title:title, icon:icon, collapsible:true, selected_tab:tab do |f|
      ...
      table_opts={ title: "მუნიციპალიტეტები &mdash; <strong>#{user.regions.count}</strong>".html_safe, icon:'/icons/region.png' }
      f.tab table_opts do |t|
        t.table_field :regions, table: table_opts do |tbl|
          tbl.table do |t|
            t.title_action admin_user_add_region_url(id:user.id), label: 'მუნიციპალიტეტის დამატება', icon: '/icons/plus.png'
